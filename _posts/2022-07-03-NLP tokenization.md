<a href="https://colab.research.google.com/github/Minsoo1036/DeepLearning-and-PyTorch/blob/main/NLP_tokenization.ipynb" target="_parent"><img src="https://colab.research.google.com/assets/colab-badge.svg" alt="Open In Colab"/></a>
...
### a. 띄어쓰기 단위 split
### b. 글자 단위 split
### c. n-gram tokenization (띄어쓰기, 글자단위 모두 적용 가능)
### d. BPE (Byte Pair Encoding, n-gram Tokenization의 계량판)
...
```
    ...
    <ipython-input-12-2722923f0583> in <module>()
          3 S4 =  '나는 책상 위에 배를 먹었다'
          4 
    ----> 5 indexed_sentence(S4.split())
    

    <ipython-input-8-101e57abb916> in indexed_sentence(sentence)
          1 def indexed_sentence(sentence):
    ----> 2     return [token2idx[token] for token in sentence]
    

    <ipython-input-8-101e57abb916> in <listcomp>(.0)
          1 def indexed_sentence(sentence):
    ----> 2     return [token2idx[token] for token in sentence]
...
# b. 글자 단위 split (a.k.a Character based tokenization)
...
print([chr(k) for k in range(int('0xAC00',16), int('0xD7A3',16)+1)]) #모든 완성형 한글 11172자
```
...
print([chr(k) for k in range(int('0x3131',16), int('0x3163',16)+1)]) #자모음
```
...
print([char2idx.get(c,0) for c in '그래서 Jason에게 사과를 했다']) #글자 단위 tokenization
```
...
```python
print([char2idx.get(c,0) for c in 'ㅇㅋ! ㄱㅅㄱㅅ']) #자모 단위 tokenization
```

    [119, 123, 3, 2, 97, 117, 97, 117]


# c. n-gram tokenization (글자 단위, 띄어쓰기 단위 모두 적용 가능)


```python
S1 = '나는 책상 위에 사과를 먹었다'

print([S1[i:i+1] for i in range(len(S1))]) # unigram
```
...
```python
print([" ".join(S1.split()[i:i+2]) for i in range(len(S1.split())-1)]) #띄어쓰기 단위 bi-gram
```




    ['나는 책상', '책상 위에', '위에 사과를', '사과를 먹었다']




```python
print([" ".join(S1.split()[i:i+3]) for i in range(len(S1.split())-2)]) #띄어쓰기 단위 tri-gram
```

    ['나는 책상 위에', '책상 위에 사과를', '위에 사과를 먹었다']
...
```python
# Algorithm1 : Learn BPE operations
...
vocab = {'l o w </w>':5, 'l o w e r </w>':2,
         'n e w e s t </w>':6,'w i d e s t </w>':3} #1번 과정 (띄어쓰기 기반의 tokenization 진행 및 단어 횟수 카운트, 이때 사전의 단어 글자는 모두 띄어 표현.)
```


```python
num_merges = 10 #BPE iteration 횟수
for i in range(num_merges): #4번 과정(repeat)
    pairs = get_stats(vocab) #2번 과정 (most frequent Characters bi-gram)
    best = max(pairs, key=pairs.get) #2번 과정 (most frequent Characters bi-gram)
    vocab = merge_vocab(best,vocab) #3번 과정 (두 글자를 합쳐 기존 사전에 반영)
    print(f'Step {i+1}')
    print(best)
    print(vocab)
    print('\\n')
...
```
    ...
    ['나는', 'ᄎ', '##ᅢᆨ', '##상', '위에', 'ᄉ', '##ᅡ', '##과', '##를', 'ᄆ', '##ᅥ', '##ᆨ', '##었다', '.', '알', '##고', 'ᄇ', '##ᅩ', '##니', '그', 'ᄉ', '##ᅡ', '##과', '##는', 'jason', '것이', '##었다', '.', '그', '##래', '##서', 'jason', '##에게', 'ᄉ', '##ᅡ', '##과', '##를', '했다']
